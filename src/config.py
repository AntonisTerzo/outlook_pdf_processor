# Configuration file for Outlook PDF Processor

# Task 1 Cities
TASK1_CITIES = [
    "TIANJIN", "SHENYANG", "AMMAN", "WUHAN", "XIAN", "SUZHOU",
    "CAIRO", "HOLON", "ABIDJAN", "ANSAN-SI", "BRAMALEA", "BRIDGEPORT",
    "DELTA", "DORVAL", "EAST TAMAKI", "FOSHAN", "GUANGZHOU", "HAYWARD",
    "KOWLOON", "MONTEVIDEO", "MUANG CHONBURI", "SIHEUNG", "SINGAPUR",
    "TROY", "TULLAMARINE"
]

# Task 2 Cities
TASK2_CITIES = [
    "LYMAN",
    "LYMAN ELECTRONICS",
    "PUNE",
    "JOHANNESBURG",
    "DALLAS",
    "BRASILIEN",
    "CHINA KRONES",
    "CHENNAI",
    "VADODARA",
    "BRASILIEN JOINVILLE",
    "MALAYSIA",
    "MALAYSIA SHA ALAM",
    "JAPAN",
    "JAPAN KYOTO",
    "KOLUMBIEN",
    "VIETNAM",
    "WELLFORD",
    "CHILE",
    "PERU",
    "ARAB. EMIRATE",
    "MEXIKO",
    "VIETNAM HANOI",
    "VIETNAM DONG NAI"
]

# Task 2 - Brasilien subcities (checked inside PDF)
BRASILIEN_SUBCITIES = [
    "INDAIATUBA",
    "RIO CLARO"
]

# Task 2 - Lyman Electronics subcities (checked inside PDF)
LYMAN_ELECTRONICS_SUBCITIES = [
    "MAXOLUTION"
]

# Task 2 - Manual review fallback cities (checked inside PDF)
MANUAL_REVIEW_FALLBACK_CITIES = [
    "TAPUKARA"
]

# Task 3 - File type identifiers (text searched inside PDF to detect type)
# Only two file types are used; the invoice file is ignored.
TASK3_FILE_TYPES = {
    "PACKING_LIST": ["LISTA DE CONTENIDO / PACKING LIST", "LISTA DE CONTENIDO/PACKING LIST"],
    "CUSTOMS_CODE": ["PARTIDAS ESTADÍSTICAS / CUSTOMS CODE", "PARTIDAS ESTADISTICAS / CUSTOMS CODE",
                     "PARTIDAS ESTADÍSTICAS/CUSTOMS CODE", "PARTIDAS ESTADISTICAS/CUSTOMS CODE"]
}
